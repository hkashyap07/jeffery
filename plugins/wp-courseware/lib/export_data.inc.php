...
function WPCW_data_handleDataExport() {
	...
	if ( isset( $_GET[ 'wpcw_export' ] ) && $exportType = $_GET[ 'wpcw_export' ] ) {
		...
		if ( ! is_user_logged_in() ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'wp_courseware' ) );
		}
...
function WPCW_data_export_quizSurveyData() {
	...
	if ( ! $quizDetails ) {
		wp_die( __( 'Sorry, could not find that survey to export the response data.', 'wp_courseware' ) );
	}

	// Ensure it's a survey
	if ( 'survey' != $quizDetails->quiz_type ) {
		wp_die( __( 'Sorry, but the selected item is not a survey, it\'s a quiz.', 'wp_courseware' ) );
	}

	// If user is not allowed to view courses, then redirect to home page
	if ( ! user_can( $current_user, 'view_wpcw_courses' ) ) {
		wp_die( __( 'You do not have sufficient permissions to export this survey data.', 'wp_courseware' ) );
	}
...
function WPCW_data_export_userImportSample() {
	...
	if ( ! user_can( $current_user, 'manage_wpcw_settings' ) ) {
		wp_die( __( 'You do not have sufficient permissions to import users.', 'wp_courseware' ) );
	}
...
function WPCW_data_export_questionImportSample() {
	...
	// Check permissions
	if ( ! user_can( $current_user, 'manage_wpcw_settings' ) ) {
		wp_die( __( 'You do not have sufficient permissions to import questions.', 'wp_courseware' ) );
	}
...
function WPCW_data_export_gradebookData() {
	...
	if ( ! $courseDetails ) {
		wp_die( __( 'Sorry, but that course could not be found.', 'wp_courseware' ) );
	}

	// Check permissions on course
	if ( ! user_can( $current_user, 'manage_wpcw_settings' ) && $current_user->ID != $courseDetails->course_author ) {
		wp_die( __( 'You do not have sufficient permissions to access this course gradebook data.', 'wp_courseware' ) );
	}
	...
	$headings = array(
		...
		__( 'Course Completion Date', 'wp_courseware' ),
	...
	foreach ( $userData as $userObj ) {
		...
		$dataToOutput[ 'completion_date' ] = __( 'n/a', 'wp_courseware' );
