...
class WPCW_UnitFrontend
{
    ...
    public function render_quizzes_showAllCorrectAnswers($returnDataInArray = false)
    {
        ...
        if ($this->unitQuizDetails->questions && count($this->unitQuizDetails->questions) > 0) {
            ...
            foreach ($this->unitQuizDetails->questions as $question) {
                ...
                // ### 3a - Question title
                $html .= sprintf(
                        '<div class="wpcw_fe_quiz_q_title">%s #%d: %s</div>', __('Question', 'wp_courseware'), $questionNum++, nl2br(htmlspecialchars($question->question_question))
                );
                        ...
                        switch ($question->question_type) {
                            ...
                            case 'upload':
                                ...
                                if (isset($this->unitQuizProgress->quiz_needs_marking_list) && is_array($this->unitQuizProgress->quiz_needs_marking_list) && in_array($question->question_id, $this->unitQuizProgress->quiz_needs_marking_list)) {
                                    $html .= sprintf(
                                            '<div class="wpcw_fe_quiz_q_result wpcw_fe_quiz_q_user_grade"><b>%s:</b>&nbsp;&nbsp;%s</div>', __('Your Grade', 'wp_courseware'), __('Pending', 'wp_courseware')
                                    );
                                    ...
                                    $html .= sprintf(
                                            '<div class="wpcw_fe_quiz_q_result wpcw_fe_quiz_q_user_grade"><b>%s:</b>&nbsp;&nbsp;%d%%</div>', __('Your Grade', 'wp_courseware'), $gradePercentage
                                    );
                                }
                                break;
                ...
                if ($setting_showUserAnswer) {
                    ...
                    if (!empty($this->unitQuizProgress->quiz_data) && isset($this->unitQuizProgress->quiz_data[$question->question_id])) {
                        ...
                        switch ($question->question_type) {
                            ...
                            case 'upload':
                                ...
                                $theirAnswer = sprintf(
                                        '<a href="%s%s" target="_blank">%s .%s %s (%s)</a>', WP_CONTENT_URL, $theirAnswerRaw, __('Open', 'wp_courseware'), pathinfo($theirAnswerRaw, PATHINFO_EXTENSION), __('File', 'wp_courseware'), WPCW_files_getFileSize_human($theirAnswerRaw)
                                );
                                break;
                    ...
                    if (!is_null($theirAnswer)) {
                        if (is_array($theirAnswer)) {
                            $html .= sprintf(
                                    '<div class="wpcw_fe_quiz_q_your_answer"><b>%s:</b>&nbsp;&nbsp;', __('Your Answer', 'wp_courseware')
                            );
                            $html .="<ul>";
                            ...
                            $html .= sprintf(
                                    '<div class="wpcw_fe_quiz_q_your_answer"><b>%s:</b>&nbsp;&nbsp;%s</div>', __('Your Answer', 'wp_courseware'), $theirAnswer
                            );
                        }
                        ...
                        $html .= sprintf(
                                '<div class="wpcw_fe_quiz_q_your_answer wpcw_fe_quiz_q_your_answer_none_found"><b>%s:</b>&nbsp;&nbsp;(%s)</div>', __('Your Answer', 'wp_courseware'), __('We don\'t have your answer for this question', 'wp_courseware')
                        );
                    }
                ...
                if ($setting_showCorrectAnswer && in_array($question->question_type, array('truefalse', 'multi'))) {
                    if (is_array($correctAnswer)) {
                        $html .= sprintf(
                                '<div class="wpcw_fe_quiz_q_correct"><b>%s:</b>&nbsp;&nbsp;', __('Correct Answer', 'wp_courseware')
                        );
                        $html .="<ul>";
                        ...
                        $html .= sprintf(
                                '<div class="wpcw_fe_quiz_q_correct"><b>%s:</b>&nbsp;&nbsp;%s</div>', __('Correct Answer', 'wp_courseware'), $correctAnswer
                        );
                    }
                ...
                if ($setting_showOtherPossible && 'multi' == $question->question_type) {
                    $html .= sprintf(
                            '<div class="wpcw_fe_quiz_q_possible"><b>%s:</b><ul class="wpcw_fe_quiz_q_possible_list">', __('Other Possible Answers', 'wp_courseware')
                    );
                ...
                if ($setting_showExplanation && $question->question_answer_explanation) {
                    $html .= sprintf(
                            '<div class="wpcw_fe_quiz_q_explanation"><b>%s:</b>&nbsp;&nbsp;%s</div>', __('Explanation', 'wp_courseware'), nl2br($question->question_answer_explanation)
                    );
                }
    ...
    public function render_completionBox_complete()
    {
        ...
        if ($this->userProgress->isCourseCompleted()) {
            ...
            if ('use_certs' == $this->parentData->course_opt_use_certificate && $certificateDetails) {
                $certHTML = sprintf(
                        '<div class="wpcw_fe_progress_box_download"><a href="%s" class="fe_btn fe_btn_download" target="_blank">%s</a></div>', WPCW_certificate_generateLink($certificateDetails->cert_access_key), __('Download Certificate', 'wp_courseware')
                );
            }
                ...
                if ('quiz_noblock' == $this->unitQuizDetails->quiz_type && 'incomplete' != $this->unitQuizProgress->quiz_paging_status) {
                    ...
                    if ($this->unitQuizProgress->quiz_needs_marking != 0) {
                        ...
                        $messageToShow .= wpautop(sprintf(
                                        __('The recommended grade for this quiz is <b>%d%%</b> (and your grade is <b>%d%%</b>). The course instructor has allowed you to retake this quiz if you wish to improve your grade.', 'wp_courseware'), $this->unitQuizDetails->show_recommended_percentage, $this->unitQuizProgress->quiz_grade
                        ));
            ...
            if ($this->unitQuizProgress->quiz_needs_marking == 0) {
                ...
                if ($this->unitQuizDetails->has_open_questions) {
                    $rtnDetails[0]['msg_overall_grade'] = sprintf(
                            __('Your grade for this quiz is <b>%d%%</b>.', 'wp_courseware'), $this->unitQuizProgress->quiz_grade
                    );
                } // Just closed questions, show out of message.
                else {
                    $rtnDetails[0]['msg_overall_grade'] = sprintf(
                            __('You got %d out of %d questions <b>(%d%%)</b> correct!', 'wp_courseware'), $this->unitQuizProgress->quiz_correct_questions, $this->unitQuizProgress->quiz_question_total, $this->unitQuizProgress->quiz_grade
                    );
                }
                ...
                if ('quiz_noblock' == $this->unitQuizDetails->quiz_type) {
                    ...
                    if ($this->check_quizzes_NonBlockingQuizOffersRetakeButton() && $attemptCount > 0) {
                        $rtnDetails[1]['msg_attempts'] = ($attemptCount == 1 ? __('You have had <b>1 previous attempt</b> at this quiz.', 'wp_courseware') : sprintf(__('You have had <b>%d previous attempts</b> at this quiz.', 'wp_courseware'), $attemptCount)
                                );
                    }
                    ...
                    if ($this->check_quizzes_hasUserPassedQuiz()) {
                        ...
                        $rtnDetails[2]['msg_attempts'] = ($attemptCount == 1 ? __('You have had <b>1 previous attempt</b> at this quiz.', 'wp_courseware') : sprintf(__('You have had <b>%d previous attempts</b> at this quiz.', 'wp_courseware'), $attemptCount)
                                );
                    }
                ...
                if ('quiz_block' == $this->unitQuizDetails->quiz_type && // Timers only appear on blocking quizzes
                    ...
                    $rtnDetails['msg_results_by_timer'] = sprintf(
                            '<b>%s:</b> %s (%s %s)', __('Completion Time', 'wp_courseware'), WPCW_time_convertSecondsToHumanLabel($this->unitQuizProgress->quiz_completion_time_seconds), WPCW_time_convertMinutesToHumanLabel($this->unitQuizDetails->quiz_timer_mode_limit), __('allowed', 'wp_courseware')
                    );
                } // end of timer check
    ...
    public function render_completionBox_quizPassStatus()
    {
        ...
        if (isset($msgDetails['button_dl_results_url']) && $switch == 'on')
        {
            $messageToShow .= sprintf(
				...
				</div>', $msgDetails['button_dl_results_url'], __('Download Results', 'wp_courseware')
            );
        }
        ...
        if ($this->check_quizzes_validQuizDetails()) {
            ...
            $html .= sprintf(
				...
				<div class="wpcw_fe_progress_box_wrap" id="wpcw_fe_unit_complete_%d">
					...
					%s</div>
				</div>', $this->unitPost->ID, WPCW_plugin_getPluginPath().'img', $this->unitPost->ID, __('Mark as Completed', 'wp_courseware'), $this->parentData->course_message_unit_pending
            );
        }
    ...
    public function render_quizzes_handleQuizRendering($showErrorsOnForm = true)
    {
        ...
        if ($this->check_paging_shouldWeShowReviewPage_rightNow()) {
            ...
            $html .= self::message_createMessage_warning(__('You can now review your answers before submitting them.', 'wp_courseware'));
        }
        ...
        if ('quiz_block' == $this->unitQuizDetails->quiz_type) {
            ...
            $html .= sprintf(
                    __('You\'ll need to correctly answer at least <b>%d of the %d</b> questions below (<b>at least %d%%</b>) to progress to the next unit.', 'wp_courseware'), $passQs, $totalQs, $this->unitQuizDetails->quiz_pass_mark
            );
            $html .= '</div>';
        }
                ...
                if (empty($this->unitQuizProgress) || 'retake_waiting' == $this->unitQuizProgress->quiz_next_step_type) {
                    ...
                    $html .= sprintf(
                            '
									...
									</div>', sprintf(__('You have <b>%s</b> to complete this quiz...', 'wp_courseware'), WPCW_time_convertMinutesToHumanLabel($this->unitQuizDetails->quiz_timer_mode_limit)), $this->unitQuizDetails->quiz_id, $this->unitPost->ID, __('Begin Quiz...', 'wp_courseware'), WPCW_plugin_getPluginPath()
                    );
                ...
                if ($this->check_paging_areWePagingQuestions()) {
                    ...
                    $html .= sprintf(
                            '<div class="wpcw_fe_paging_progress">%s</div>', sprintf(
                                    __('Question %d of %d', 'wp_courseware'), $questionToShowIndex + 1, $this->fetch_paging_getQuestionCount()
                            )
                    );
                    ...
                    if ($this->check_paging_shouldWeShowAnswerLaterButton()) {
                        $html .= sprintf(
                                '<div class="wpcw_fe_quiz_answer_later"><a href="#" class="fe_btn fe_btn_small fe_btn_navigation" id="wpcw_fe_quiz_answer_later">%s</a></div>', __('Answer Later...', 'wp_courseware')
                        );
                    }
                    ...
                    if ($this->check_paging_shouldWeShowPreviousButton()) {
                        $buttonPreviousClicker = sprintf(
                                '<input type="submit" class="fe_btn fe_btn_completion btn_completion" id="fe_btn_quiz_previous" name="previous_question" value="%s">', __('&laquo; Previous Question', 'wp_courseware')
                        );
                    }
    ...
    public function render_navigation_getNavigationBox()
    {
        ...
        if ($nextAndPrev['prev'] > 0) {
            $html .= sprintf(
                    '<a href="%s" class="fe_btn fe_btn_navigation">&laquo; %s</a> ', get_permalink($nextAndPrev['prev']), __('Previous Unit', 'wp_courseware')
            );
        }
        ...
        if (!$disableNextButton && $nextAndPrev['next'] > 0) {
            ...
            $html .= sprintf(
                    '<a href="%s" class="fe_btn fe_btn_navigation %s">%s &raquo;</a>', get_permalink($nextAndPrev['next']), false, __('Next Unit', 'wp_courseware')
            );
        }
        ...
        if (!empty($tagBucketList)) {
            foreach ($tagBucketList as $tagID => $tagDetails) {
                ...
                if ($tagDetails['question_open_count'] > 0) {
                    $html[] = sprintf(
                            '<b>%s:</b> %s', $tagDetails['tag_details']->question_tag_name, sprintf(__('Your grade is %d%%', 'wp_courseware'), $tagDetails['score_total'])
                    );
                } // Just closed questions, show out of message.
                else {
                    $html[] = sprintf(
                            '<b>%s:</b> %s', $tagDetails['tag_details']->question_tag_name, sprintf(
                                    __('%d out of %d correct (%d%%)', 'wp_courseware'), $tagDetails['score_correct_questions'], $tagDetails['question_count'], $tagDetails['score_total']
                            )
                    );
    ...
    {
        return self::message_createMessage_error(__('An error occured, so your unit progress was not updated. Please refresh the page and try again.', 'wp_courseware'));
    }
     ...
     */
    public function fetch_quizzes_extractQuizStatusFromAnswers($checkedAnswerList)
    {
        $fullDetails = array();
     ...
     */
    public function update_quizzes_saveUserProgress_completeQuiz($resultDetails, $checkedAnswerList)
    {
        global $wpdb, $wpcwdb;
        $wpdb->show_errors();
    ...
    public function check_quizzes_canWeContinue_checkAnswersFromOnePageQuiz($potentialAnswers)
    {
        ...
        if ($this->unitQuizDetails->questions && $answerCountSoFar < count($this->unitQuizDetails->questions) && !$this->check_timers_doWeHaveAnActiveTimer_thatHasExpired()) {
            ...
            echo self::message_createMessage_error(__('Please provide an answer for all of the questions on this page.', 'wp_courseware'));
    ...
    public function check_quizzes_gradeQuestionsForQuiz()
    {
        ...
        if ('survey' == $this->unitQuizDetails->quiz_type) {
            ...
            echo self::message_createMessage_success(__('Thank you for your responses. This unit is now complete.', 'wp_courseware'));
