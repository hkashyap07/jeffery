...
class WPCW_quiz_base {
    ...
    public function getSection_processHeader($columnCount) {
        ...
        // Quiz type
        $html .= sprintf(
                '<td colspan="%d">%s: <b>%s</b>', $columnCount, __('Type', 'wp_courseware'), WPCW_quizzes_getQuestionTypeName($this->questionType)
        );

        // Usage count
        if (!$this->hideQuestionUsageCount) {
            $html .= sprintf(
                    '<span class="wpcw_quiz_row_header_usage_count">(%s)</span>', sprintf(_n('In use by 1 quiz', 'In use by %d quizzes', $this->quizItem->question_usage_count, 'wp_courseware'), $this->quizItem->question_usage_count)
            );
        }
    ...
    public function getSection_questionTags($columnCount, $questionNotSavedYet = false) {
        ...
        $html .= sprintf(
                '<th>%s<span class="wpcw_inner_hint">%s</span></th>', __('Question Tags', 'wp_courseware'), __('(Optional) Add tags to this question to add it to your pool of questions to reuse in other quizzes.', 'wp_courseware')
        );
    ...
    public function getSection_Explanation($columnCount) {
        $html = '<tr>';
        $html .= sprintf(
                '<th>%s<span class="wpcw_inner_hint">%s</span></th>', __('Explanation', 'wp_courseware'), __('(Optional) Displayed after the quiz is submitted to offer information on the correct answer.', 'wp_courseware')
        );
    ...
    public function getSection_Hint($columnCount, $messageToShow) {
        $html = '<tr>';
        $html .= sprintf(
                '<th>%s<span class="wpcw_inner_hint">%s</span></th>', __('Hint', 'wp_courseware'), $messageToShow
        );
    ...
    public function getSection_showImageField($columnCount) {
        $html = '<tr>';
        $html .= sprintf(
                '<th>%s<span class="wpcw_inner_hint">%s</span></th>', __('Question Image URL', 'wp_courseware'), __('(Optional) An image to show with the question.', 'wp_courseware')
        );
    ...
    protected function renderForm_toString_withClass($parentQuiz, $questionNum, $selectedAnswer, $showAsError, $cssClass, $errorToShow = false) {
        ...
        // Question title
        $html .= sprintf(
                '<div class="wpcw_fe_quiz_q_title">%s #%d: %s%s</div>', __('Question', 'wp_courseware'), $questionNum, nl2br(htmlspecialchars($this->quizItem->question_question)), ($wrongAnswerState ? '<span class="wpcw_fe_quiz_status">(' . __('Incorrect', 'wp_courseware') . ')</span>' : '')
        );
        ...
        if ($this->answerList) {
            ...
            foreach ($this->answerList as $answerItem => $answerValue) {
                ...
                if ($flagMultAns || $parentQuiz->quiz_type == 'survey' && $this->questionType != 'truefalse') {
                    ...
                    $html .= sprintf(
                            '<li><input type="checkbox" name="%s[]" id="%s_%s" value="%s" %s> <label for="%s_%s">%s</label> %s</li>', $fieldID, // Name
                    ...
                    $html .= sprintf(
                            '<li><input type="radio" name="%s" id="%s_%s" value="%s" %s> <label for="%s_%s">%s</label> %s</li>', $fieldID, // Name
                            $fieldID, sanitize_title($answerValue), // CSS ID
                            $answerValue, // Value
                            $checked, // Mark the correct item as checked
                            $fieldID, sanitize_title($answerValue), // Label for=""
