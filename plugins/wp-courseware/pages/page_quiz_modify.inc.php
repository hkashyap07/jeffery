...
function WPCW_showPage_ModifyQuiz_load() {
	...
	if ( $quizDetails = WPCW_quizzes_getQuizDetails( $quizID, false, false, false ) ) {
		...
		if ( ! $quizDetails ) {
			...
			$cantEditQuizMsg = apply_filters( 'wpcw_permissions_user_can_edit_quiz_msg', esc_attr__( 'You are not permitted to edit this quiz.', 'wp_courseware' ), $current_user, $quizDetails );
		...
		$cantAddQuizMsg = apply_filters( 'wpcw_permissions_user_can_add_quiz_msg', esc_attr__( 'You are not permitted to add a new quiz.', 'wp_courseware' ), $current_user );

		// Legacy Fitler
		$canAddQuiz     = apply_filters( 'wpcw_back_permissions_user_can_add_quiz', $canAddQuiz, $current_user->ID );
		$cantAddQuizMsg = apply_filters( 'wpcw_back_msg_permissions_user_can_add_quiz', esc_attr__( 'You are not permitted to add a new quiz.', 'wp_courseware' ), $current_user->ID );
	...
	$tabList = array(
		...
		'wpcw_section_break_quiz_results'         => array(
			'label'    => __( 'Result Settings', 'wp_courseware' ),
			'cssclass' => 'wpcw_quiz_only_tab'
		),
		'wpcw_section_break_quiz_custom_feedback' => array( 'label'    => __( 'Custom Feedback', 'wp_courseware' ),
		                                                    'cssclass' => 'wpcw_quiz_only_tab'
		),
	...
	$formDetails = array(
		...
		'quiz_attempts_allowed' => array(
			...
			'desc'     => __( 'The maximum number of attempts that a trainee is given to complete a quiz.', 'wp_courseware' ),
		),
		...
		'quiz_results_downloadable' => array(
			'label'      => __( 'Downloadable Quiz Results?', 'wp_courseware' ),
			'type'       => 'checkbox',
			'required'   => false,
			'cssclass'   => 'wpcw_quiz_only',
			'extralabel' => __( 'Enabling this option will provide a button for students to download their quiz results in PDF format upon completing a quiz.', 'wp_courseware' ),
		),
		...
		'quiz_author' => array(
			'label' => __( 'Quiz Author', 'wp_courseware' ),
			'type'  => 'hidden'
		),
	);
...
function WPCW_showPage_customFeedback_showEditForms($quizID, $page)
{
	...
	printf('<p>(%s) %s</p>',
		...
		__('If you are using question tags within your quiz, you can create custom messages to display upon submission of the quiz based on a student\'s results for a particular tag.', 'wp_courseware')
	);
