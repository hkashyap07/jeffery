...
function WPCW_showPage_CourseOrdering_load()
{
	...
	if ( ! user_can( $current_user, 'view_wpcw_courses' ) ) {
		$page->showMessage( __( 'Sorry, but you do not have permission to re-order course modules and units.', 'wp_courseware' ), true );
		$page->showPageFooter();
	...
	if ( ! user_can( $current_user, 'manage_wpcw_settings' ) && $current_user->ID != $courseDetails->course_author ) {
		$page->showMessage( __( 'Sorry, but you do not have permission to re-order the modules and units of this course.', 'wp_courseware' ), true );
		$page->showPageFooter();
