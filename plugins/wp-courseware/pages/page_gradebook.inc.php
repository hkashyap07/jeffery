...
function WPCW_showPage_GradeBook_load() {
	...
	if ( ! user_can( $current_user, 'view_wpcw_courses' ) || ( ! user_can( $current_user, 'manage_wpcw_settings' ) && isset( $courseDetails->course_author ) && $courseDetails->course_author != $current_user->ID ) ) {
		$page->showPageHeader( __( 'GradeBook', 'wp_courseware' ), '75%', WPCW_icon_getPageIconURL() );
		$page->showMessage( __( 'Sorry, but you cannot access this course gradebook.', 'wp_courseware' ), true );
		$page->showPageFooter();
	...
	if ( $quizzesForCourse ) {
		...
		// ### Create heading for cumulative data.
		$tblCol              = new TableColumn( __( 'Course Completion Date', 'wp_courseware' ), 'completion_date' );
		$tblCol->headerClass = "wpcw_center";
		$tblCol->cellClass   = "wpcw_grade_summary wpcw_center";
		$tbl->addColumn( $tblCol );
	...
	if ( ! $userList ) {
		...
		foreach ( $userList as $singleUser ) {
			...
			if ( $usingCertificates && $certDetails = WPCW_certificate_getCertificateDetails( $singleUser->ID, $courseDetails->course_id, false ) ) {
				$data[ 'certificate_available' ] = sprintf( '<a href="%s" title="%s" target="_blank">%s</a>', WPCW_certificate_generateLink( $certDetails->cert_access_key ), __( 'Download the certificate for this user.', 'wp_courseware' ), __( 'View Certificate', 'wp_courseware' ) );
			} else {
