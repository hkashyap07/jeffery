...
function WPCW_quizzes_handleQuestionDeletion($page)
{
	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['question_id']))
	{
		...
		if ($questionDetails)
		{
			...
			$cantEditQuestionMsg = apply_filters( 'wpcw_permissions_user_can_edit_question_msg', esc_attr__( 'You are not permitted to delete this question.', 'wp_courseware' ), $current_user, $questionDetails );
...
function WPCW_showPage_QuestionPool_processActionForm($page)
{
	...
	foreach ( $validatedQuestions as $questionId ) {
		...
		$cantBulkEditQuestionMsg = apply_filters( 'wpcw_permissions_user_can_bulkedit_question_msg',
			sprintf( esc_attr__( 'You are not permitted to bulk edit Question ID: %s.', 'wp_courseware' ), $questionId ),
			$current_user,
			$questionId
		);
