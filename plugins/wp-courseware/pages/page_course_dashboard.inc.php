...
function WPCW_showPage_Dashboard_load()
{
	...
	if ($courses || $searchString)
	{
		...
		<p class="search-box">
			<label class="screen-reader-text" for="wpcw_courses_search_input"><?php _e('Search Courses', 'wp_courseware'); ?></label>
			<input id="wpcw_courses_search_input" type="text" value="<?php echo $searchString ?>" name="s"/>
			<input class="button" type="submit" value="<?php _e('Search Courses', 'wp_courseware'); ?>"/>
		...
		{
			$tbl->addRowObj(new RowDataSimple('wpcw_center wpcw_none_found', __('There are currently no courses to show.', 'wp_courseware'), 7));
		}
